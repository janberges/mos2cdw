...
for driver.n in np.arange(np.ceil(
    ...
    optimize()
    driver.to_xyz('relax_27_inward_%d.xyz' % driver.n)

    info('Outward triangle')

    triangle(atoms=[1093, 1174, 1177], amplitude=-0.2)
    optimize()
    driver.to_xyz('relax_27_outward_%d.xyz' % driver.n)

    info('Triangle CDW')

    triangles()
    optimize()
    driver.to_xyz('relax_27_cdw_%d.xyz' % driver.n)
